...
interpolates as the current year). These "useful features" can introduce ambiguity and errors in your data. 
...
for example names or identifiers like MAR1, DEC1, OCT4. So if you are avoiding the date format overall, 
it is easier to identify these issues. 

To avoid ambiguity between regional differences in date formatting and compatibility across spreadsheet 
software programs or when exporting data to text based formats such as CSV, 
...
This serial number represents the number of days from December 31, 1899. In the example, July 2, 2014 is stored as 
the serial number 41822.

> ## Excel's date systems on Macs
> Excel also entertains a second date system, the 1904 date system, as the default in Excel for Macintosh. 
...
the date above as the 7th day of December, however a researcher from the US will interpret the same entry as the 
12th day of July. This regional variation is handled automatically by your
...
> > Fro the cell that is selected in the figure above, we can see that Excel has stored the date "08/01/2015" 
> > (i.e. 8th January 2015 using the DD/MM/YYYY format).
> > You can see that even though we expected the year to be 2014 (this was data for year 2014), 
> > the year is actually 2015. What happened here is 
> > that the field assistant who collected the data for year 2014 initially forgot to include their data for 'plot 3' 
...
> > format because it contains multiple sheets." Choose `Save Active Sheet`. 
> > 4. Make sure you close your current messy data spreadsheet for now (you can reopen it later).   
...
Entering dates in one cell is helpful but due to the fact that the spreadsheet programs may interpret and save the 
data in different ways (doing that somewhat behind the scenes), there is a better practice. In dealing with dates in 
...
question, this might be what is useful to you, and there is practically no possibility for ambiguity creeping in.
...
So, can you convert all your dates into DOY format? For Excel, here is a useful guide:
...
knowing the format they can then be correctly processed by the receiving data analysis
software.
