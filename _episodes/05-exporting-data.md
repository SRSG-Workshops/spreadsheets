...
- Excel's `*.xls` or `*.xlsx` are proprietary file formats, and it is possible that in
  ...
  rare) to make it inconvenient, if not impossible, to open the file. `*.xls` is already an older file
...
environments, such as SQLite and R. We are not tied to a certain version of a certain expensive program when we work with CSV files, so
it is a
...
You can open CSV files back in Excel!
{: .callout}
...
> commas within datasets see [the discussion
> page](/discuss).
{: .callout}
