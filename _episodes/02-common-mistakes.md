...
Your data sheet might get very long over the course of the experiment. This makes it harder to enter data if you cannot see your headers
at the top of the spreadsheet. But do not repeat your header row. These can easily get mixed into the data, 
...
However, there is a difference between a zero and a blank cell in a spreadsheet. To the computer, a zero is actually data. You measured
...
(because the computer can't guess the value of the missing observations). Because of this, it is very important to record zeros as zeros and truly missing data as nulls.
...
Sometimes confusing null values are automatically recorded from the measuring device. If that's the case, there is not 
much you can do, but it can be addressed in data cleaning with a tool like 
[OpenRefine](http://www.datacarpentry.org/OpenRefine-ecology-lesson/) before analysis. Other times different null 
...
that these are intended to represent missing (null) values. How these values are interpreted will depend on 
...
**Solution**: Do not include more than one piece of information in a cell. This will limit the ways in which you can analyze your data. 
...
Underscores (`_`) are a good alternative to spaces. Consider writing names in camel case 
...
**Example**: You treat your spreadsheet program as a word processor when writing notes. For example, copying 
data directly from Microsoft Word or other applications.
...
etc. in their spreadsheet.  Also, when copying data in from applications such as Microsoft Word, formatting and fancy non-standard characters (such
